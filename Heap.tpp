...
Heap<T>::Heap(std::vector<T> start_values) {
  ...
  for (std::vector<T>::size_type i = 0; i < start_values.size(); i++) {
...
void Heap<T>::insert(T value) {
  ...
  while (index > 0 && values.at(parent_index) > values.at(index)) {
...
void Heap<T>::heapify(int parent_index) {
  ...
  if (std::vector<T>::parent_index < 0 || parent_index >= values.size()) return;

  // find children indexes
std::vector<T>::left_child_index = parent_index * 2 + 1;
  std::vector<T>:: right_child_index = parent_index * 2 + 2;
