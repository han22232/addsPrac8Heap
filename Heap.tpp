...
void Heap<T>::insert(T value)
{
    ...
    // While index is not the root and parent is greater than the value
    while (index > 0 && values[(index - 1) / 2] > values[index])
    {
        // Swap parent and current value
        swap(values[(index - 1) / 2], values[index]);
...
void Heap<T>::remove(T value)
{
    ...
    // Remove the last element (value)
    values.pop_back();

    // Heapify the heap from the index
    heapify(index);
}
